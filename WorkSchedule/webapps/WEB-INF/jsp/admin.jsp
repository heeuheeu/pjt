          ...
          <tbody>
            <c:forEach items="${divlist}" var="divlist">	
		  		<tr>
					...
					<td> <input type="checkbox" id="divchk" onclick="divCheck('${divlist.divid}',this)"  
					 <c:if test="${divlist.checkyn eq 'Y'}" > checked </c:if>> </td>	    
				</tr>
		     </c:forEach>
          </tbody>
        </table>
        		<input type="text" class="form-control" placeholder="사업부 추가" size=90px name=divname required>
				<input type="submit" class="btn btn-default" role="button" value="추가">				
        </form>
...
<!-- //////////////////////////////////////////////////////////////////////////////////////////////////////     -->    
        ...
        <form role="form"  name="deptadd" onsubmit="return checkdeptname()" class="form-inline" action="deptAdd.inc"	method="post">
        <table>
          <thead>
            <tr>
              <td>부서 번호</td>	
          ...
          <tbody>
           <c:forEach items="${deptList}" var="deptList">	
			  		<tr>
						...
						<td> <input type="checkbox" name="deptchk" id="deptchk" onclick="deptCheck('${deptList.deptid}',this)" <c:if test="${deptList.checkyn eq 'Y'}" > checked </c:if>> </td>	    
					</tr>
			</c:forEach>	
          </tbody>
        </table>
        	<input type="text" class="form-control" placeholder="팀 추가" size=50px name="deptname"  required> 
...
<!-- //////////////////////////////////////////////////////////////////////////////////////////////////////     -->   
        ...
        <form role="form" class="form-inline" action="locAdd.inc"	name="locadd" method="post"  onsubmit="return checklocname()">
        <table>
          <thead>
            <tr>
              <td>근무지 번호</td>
          ...
          <tbody>
          		...
          		<c:forEach items="${locList}" var="locList">	
			  		<tr>
						...
						<td><input type="checkbox" name="locchk" onclick="locCheck('${locList.locid}',this)"  <c:if test="${locList.checkyn eq 'Y'}" > checked </c:if>></td>	    
					</tr>
			    </c:forEach>	
			    
          </tbody>
        </table>
        			<input type="text" class="form-control" placeholder="근무지 추가" size=90px name="locname" id="locname" required>
					<input type="submit" class="btn btn-default" role="button" value="추가">
	...
	function checkdivname() {		
		 ...
		 <c:forEach items="${divlist}" var="divlist">		
		  	...
		  	var obj = {divid : divid , divname : divname , checkyn : checkyn};
		  	lists.push(obj);
		  </c:forEach> 		
	...
	function checklocname() { 		
		 ...
		 <c:forEach items="${locList}" var="locList">	
		  	...
		  	var obj = {locid : locid , locname : locname , checkyn : checkyn};
		  	lists.push(obj);
		  </c:forEach> 		
	...
	function checkdeptname() {	
		 ...
		 <c:forEach items="${deptList}" var="deptList">			
		  	...
		  	var obj = {divname : divname , deptname : deptname};
		  	lists.push(obj);
		 </c:forEach> 		

		 	for(var i=0; i<lists.length; i++){
	 			if(deptadd.deptname.value == lists[i].deptname && deptadd.divname.value == lists[i].divname ){
