...
function Flatpickr(element, config) {
	...
	function setupMobile() {
		...
		self.mobileFormatStr = inputType === "datetime-local" ? "Y년 m월 d일\\TH:i:S" : inputType === "date" ? "Y년 m월 d일" : "H:i:S";

		if (self.selectedDates.length) {
			self.mobileInput.defaultValue = self.mobileInput.value = formatDate(self.mobileFormatStr, self.selectedDates[0]);
		}

		if (self.config.minDate) self.mobileInput.min = formatDate("Y년 m월 d일", self.config.minDate);

		if (self.config.maxDate) self.mobileInput.max = formatDate("Y년 m월 d일", self.config.maxDate);
